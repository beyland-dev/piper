...
export default function composableOAuthWorkflow() {
	return withTests({
		...
		steps: withRiskReview({
			...
			reviewGoal:
				"Review the OAuth changes for token handling, rollback readiness, and supportability",
			reviewOutput: "oauth-review",
			steps: withImplementationPlan({
				planningGoal:
					"Create a shared implementation plan for an OAuth login and token refresh rollout",
