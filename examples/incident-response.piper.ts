...
export default function incidentResponseWorkflow() {
	return workflow(
		...
		parallel(
			{
				status:
					"Investigating the incident, preparing a hotfix, and drafting stakeholder guidance...",
			},
			task({
				...
				validate: [
					runtimeValue(
						async ({ readArtifact }) =>
							(await readArtifact("incident-brief")).toLowerCase().includes("root cause"),
			...
			protect(
				{
					...
					validate: [
						runtimeValue(
							async ({ readArtifact }) =>
								(await readArtifact("postmortem-outline")).trim().length > 40,
							"postmortem outline exists",
						),
					],
				},
				task({
