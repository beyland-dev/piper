...
export default function oAuthWorkflow() {
	return recover(
		...
		task({
			...
			harness: "pi",
			context: ["The codebase uses Express and PostgreSQL.", "The auth module lives in src/auth/."],
			artifact: "plan",
		}),
		parallel(
		...
		protect(
			...
			task({
				...
				harness: "pi",
				context: [
					"Look for token leakage, missing CSRF protection, and insecure redirect handling.",
				],
			}),
		),
	);
